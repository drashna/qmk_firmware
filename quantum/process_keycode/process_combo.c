...
#    define ACTIVATE_COMBO(combo) \
        do { combo->active = true; } while (0)
#    define DEACTIVATE_COMBO(combo) \
        do { combo->active = false; } while (0)
#    define DISABLE_COMBO(combo) \
        do { combo->disabled = true; } while (0)
...
#    define ACTIVATE_COMBO(combo) \
        do { combo->state |= 0x80; } while (0)
#    define DEACTIVATE_COMBO(combo) \
        do { combo->state &= ~0x80; } while (0)
#    define DISABLE_COMBO(combo) \
        do { combo->state |= 0x40; } while (0)
#    define RESET_COMBO_STATE(combo) \
        do { combo->state &= ~0x7F; } while (0)
#endif
...
static inline uint16_t _get_wait_time(uint16_t combo_index, combo_t *combo) {
    if (_get_combo_must_hold(combo_index, combo)
        ...
        if (longest_term < COMBO_HOLD_TERM) { return COMBO_HOLD_TERM; }
    }
...
void clear_combos(void) {
    ...
    for (index = 0; index < COMBO_LEN; ++index) {
        ...
        if (!COMBO_ACTIVE(combo)) { RESET_COMBO_STATE(combo); }
    }
}

static inline void dump_key_buffer(void) {
    if (key_buffer_size == 0) { return; }
    ...
    for (uint8_t key_buffer_i = 0; key_buffer_i < key_buffer_size; key_buffer_i++) {
        ...
        if (IS_NOEVENT(record->event)) { continue; }
...
void drop_combo_from_buffer(uint16_t combo_index) {
    ...
    while (i != combo_buffer_write) {
        ...
        if (qcombo->combo_index == combo_index) {
            ...
            if (i == combo_buffer_read) { INCREMENT_MOD(combo_buffer_read); }
            break;
        }
        INCREMENT_MOD(i);
    }
...
static bool process_single_combo(combo_t *combo, uint16_t keycode, keyrecord_t *record, uint16_t combo_index) {
    ...
    if (-1 == (int16_t)key_index) { return false; }
    ...
    if (record->event.pressed && !COMBO_DISABLED(combo)) {
        ...
        if (!COMBO_ACTIVE(combo)) {
            ...
            if (longest_term < time) { longest_term = time; }
        }
...
#    ifdef COMBO_PROCESS_KEY_RELEASE
                if (process_combo_key_release(combo_index, combo, key_index, keycode)) { release_combo(combo_index, combo); }
#    endif
            }
#endif
