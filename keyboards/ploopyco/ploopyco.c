...
#ifndef OPTICAL_ENCODER_DEBOUNCE
#    define OPTICAL_ENCODER_DEBOUNCE 5
#endif
#ifndef OPTICAL_SCROLL_BUTT_DEBOUNCE
#    define OPTICAL_SCROLL_BUTT_DEBOUNCE 100
#endif
...
static pin_t optical_encoder_pin_a[NUM_ENCODERS_MAX_PER_SIDE] = ENCODERS_PAD_A;
static pin_t optical_encoder_pin_b[NUM_ENCODERS_MAX_PER_SIDE] = ENCODERS_PAD_B;

void encoder_driver_init(void) {
    setPinInput(optical_encoder_pin_a[0]);
    setPinInput(optical_encoder_pin_b[0]);
    opt_encoder_init();
}
...
void encoder_driver_task(void) {
    ...
    uint16_t p1 = analogReadPin(optical_encoder_pin_a[0]);
    uint16_t p2 = analogReadPin(optical_encoder_pin_b[0]);
    ...
    if (timer_elapsed(last_mid_click) < OPTICAL_SCROLL_BUTT_DEBOUNCE) {
        return;
    }

    // Limit the number of scrolls per unit time.
    if (timer_elapsed(last_scroll) < OPTICAL_ENCODER_DEBOUNCE) {
        return;
    }
