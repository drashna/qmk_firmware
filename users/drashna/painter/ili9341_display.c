...
void draw_ui_user(void) {
    ...
    if (is_keyboard_left()) {
        ...
        if (hue_redraw || last_keymap_config.raw != keymap_config.raw) {
            ...
            temp_pos = qp_drawtext_recolor(ili9341_display, xpos, ypos, font, buf1, last_user_config.autocorrection ? 153 : 255, 255, 255, last_user_config.autocorrection ? 153 : 255, 255, 0);
            xpos += MAX(temp_pos, 30);
...
void keyboard_post_init_quantum_painter(void) {
    ...
    ili9341_display = qp_ili9341_make_spi_device(240, 320, DISPLAY_CS_PIN_ILI9341, DISPLAY_DC_PIN, DISPLAY_RST_PIN_ILI9341, 4, 0);
