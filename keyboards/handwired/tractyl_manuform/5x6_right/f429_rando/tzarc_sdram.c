...
void SDRAM_Init(void) {
    ...
    static const struct {
        ...
        // Control signals (SDCKE0, SDNWE0, SDNRAS, SDNCAS, SDCLK, SDCS0)
        {GPIOC, 3},  {GPIOC, 0},  {GPIOF, 11}, {GPIOG, 15},
        {GPIOG, 8},  {GPIOC, 2},
        // Byte masks
        {GPIOE, 0},  {GPIOE, 1}
    };
    ...
    uint32_t sdcr_config = (1 << FMC_SDCR1_RPIPE_Pos) |  // RPIPE: 1 HCLK clock cycle delay
                           ...
                           (2 << FMC_SDCR1_NR_Pos) |     // NR: 12-bit row address
    ...
    FMC_Bank5_6->SDCMR = (7 << FMC_SDCMR_NRFS_Pos) | // NRFS: Number of auto-refresh: 8
                         (1 << FMC_SDCMR_CTB2_Pos) | // CTB1: Target Bank 2n
