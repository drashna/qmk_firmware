...
static const SDRAMConfig sdram_cfg = {
    ...
    .sdtr = (uint32_t)((2 - 1) |   // FMC_LoadToActiveDelay = 2 (TMRD: 2 Clock cycles)
                       ((7) << 4) |  // FMC_ExitSelfRefreshDelay = 5 (TXSR: min=70ns (5x11.11ns))
                       ((4) << 8) |  // FMC_SelfRefreshTime = 4 (TRAS: min=42ns (4x11.11ns) max=120k (ns))
                       ((7) << 12) | // FMC_RowCycleDelay = 5 (TRC:  min=50 (5x11.11ns))
   ...
   // clang-format off
    ...
    /* STM32_SYSCLK = 168MHz, FMC_SDClock_Period_2 -> SDCLK = 84MHz
       W9825G6KH: 64ms / 8192 = 7.8125us
       7.8125us * 84MHz = 656 - 20 = 636 */
...
static void sdram_bulk_erase(void) {
    ...
    volatile uint8_t *end = p + W9825G6KH_SIZE;
    while (p < end)
        *p++ = 0xFF;
}

/**
 * @brief Checks the whole SDRAM bank for 0xFF values.
 *
 */
static void sdram_bulk_check(void) {
    volatile uint8_t *p   = (volatile uint8_t *)SDRAM_BANK_ADDR;
    volatile uint8_t *end = p + W9825G6KH_SIZE;
    while (p < end)
        if (*p++ != 0xFF) {
            dprintf("SDRAM Init: Bulk Check Failed\n");
            return;
        }
    dprintf("SDRAM Init: Bulk Check Passed\n");
}
...
const pin_t fmc_sdram_pin_array[] = {
    FMC_SDRAM_D0_PIN, FMC_SDRAM_D1_PIN, FMC_SDRAM_D2_PIN, FMC_SDRAM_D3_PIN, FMC_SDRAM_D4_PIN, FMC_SDRAM_D5_PIN, FMC_SDRAM_D6_PIN,
    FMC_SDRAM_D7_PIN, FMC_SDRAM_D8_PIN, FMC_SDRAM_D9_PIN, FMC_SDRAM_D10_PIN, FMC_SDRAM_D11_PIN, FMC_SDRAM_D12_PIN, FMC_SDRAM_D13_PIN,
    FMC_SDRAM_D14_PIN, FMC_SDRAM_D15_PIN, FMC_SDRAM_A0_PIN, FMC_SDRAM_A1_PIN, FMC_SDRAM_A2_PIN, FMC_SDRAM_A3_PIN, FMC_SDRAM_A4_PIN,
    FMC_SDRAM_A5_PIN, FMC_SDRAM_A6_PIN, FMC_SDRAM_A7_PIN, FMC_SDRAM_A8_PIN, FMC_SDRAM_A9_PIN, FMC_SDRAM_A10_PIN, FMC_SDRAM_A11_PIN,
    FMC_SDRAM_A12_PIN, FMC_SDRAM_BA0_PIN, FMC_SDRAM_BA1_PIN, FMC_SDRAM_SDNRAS_PIN, FMC_SDRAM_SDCLK_PIN, FMC_SDRAM_SDNCAS_PIN,
    FMC_SDRAM_SDNWE_PIN, FMC_SDRAM_SDNE_PIN, FMC_SDRAM_SDCKE_PIN, FMC_SDRAM_NBL0_PIN, FMC_SDRAM_NBL1_PIN,
};
...
void board_init(void) {
    ...
    dprintf("SDRAM Init: Pins\n");

    sdramInit();
    sdramStart(&SDRAMD1, &sdram_cfg);
    dprintf("SDRAM Init: Device\n");
    dprintf("SDRAM Init: Full Erase Start\n");
    sdram_bulk_erase();
    dprintf("SDRAM Init: Full Erase Finish\n");
    sdram_bulk_check();
}
