...
const key_override_t *clear_active_override(const bool allow_reregister) {
    if (active_override == NULL) { return NULL; }
    ...
    if (active_override->custom_action != NULL) { unregister_replacement &= active_override->custom_action(false, active_override->context); }
...
static bool try_activating_override(const uint16_t keycode, const uint8_t layer, const bool key_down, const bool is_mod, const uint8_t active_mods, bool *activated) {
    if (key_overrides == NULL) { return true; }
    ...
    for (uint8_t i = 0;; i++) {
        ...
        // Try firing the custom handler
        if (override->custom_action != NULL) { register_replacement &= override->custom_action(true, override->context); }
...
bool process_key_override(const uint16_t keycode, const keyrecord_t *const record) {
    ...
    if (key_down) {
        switch (keycode) {
            case KEY_OVERRIDE_TOGGLE: key_override_toggle(); return false;

            case KEY_OVERRIDE_ON: key_override_on(); return false;

            case KEY_OVERRIDE_OFF: key_override_off(); return false;
    ...
    if (is_mod || key_down) {
        ...
        if (!send_key_action) { send_keyboard_report(); }
    }
    ...
    if (!activated && active_override != NULL) {
        if (is_mod) {
            ...
            if (should_deactivate) { clear_active_override(false); }
        }
    }
