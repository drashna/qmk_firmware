...
__attribute__((weak)) bool auto_mouse_activation(report_mouse_t mouse_report) {
    ...
    return abs(auto_mouse_context.total_mouse_movement.x) > AUTO_MOUSE_THRESHOLD || abs(auto_mouse_context.total_mouse_movement.y) > AUTO_MOUSE_THRESHOLD || abs(auto_mouse_context.total_mouse_movement.h) > AUTO_MOUSE_WHEEL_THRESHOLD || abs(auto_mouse_context.total_mouse_movement.v) > AUTO_MOUSE_WHEEL_THRESHOLD || mouse_report.buttons;
}
...
void auto_mouse_reset_trigger(bool pressed) {
    if (pressed) {
#    ifdef LAYER_LOCK_ENABLE
        if (is_layer_locked(AUTO_MOUSE_DEFAULT_LAYER)) {
            return;
        }
#    endif
        if (layer_state_is((AUTO_MOUSE_TARGET_LAYER))) {
