...
static td_state_t cur_dance(qk_tap_dance_state_t *state) {
    if (state->count == 1) {
...
void qk_tap_dance_quatrain_finished(qk_tap_dance_state_t *state, void *user_data) {
    qk_tap_dance_quatrain_t *quatrain = (qk_tap_dance_quatrain_t *)user_data;
    quatrain->tap.state = cur_dance(state);

    switch (quatrain->tap.state) {
        case TD_SINGLE_TAP: register_code(quatrain->kc1); break;
        case TD_SINGLE_HOLD: register_code(quatrain->kc2); break;
        case TD_DOUBLE_TAP: register_code(quatrain->kc3); break;
        case TD_DOUBLE_HOLD: register_code(quatrain->kc3); break;
        // Last case is for fast typing. Assuming your key is `f`:
        // For example, when typing the word `buffer`, and you want to make sure that you send `ff` and not `Esc`.
        // In order to type `ff` when typing fast, the next character will have to be hit within the `TAPPING_TERM`, which by default is 200ms.
        case TD_DOUBLE_SINGLE_TAP: tap_code(quatrain->kc1); register_code(quatrain->kc1); break;
        default: break;
    }
}
...
void qk_tap_dance_quatrain_reset(qk_tap_dance_state_t *state, void *user_data) {
    ...
    switch (quatrain->tap.state) {
        case TD_SINGLE_TAP: unregister_code(quatrain->kc1); break;
        case TD_SINGLE_HOLD: unregister_code(quatrain->kc2); break;
        case TD_DOUBLE_TAP: unregister_code(quatrain->kc3); break;
        case TD_DOUBLE_HOLD: unregister_code(quatrain->kc4); break;
        case TD_DOUBLE_SINGLE_TAP: unregister_code(quatrain->kc1); break;
        default: break;
    }
    quatrain->tap.state = TD_NONE;
}
