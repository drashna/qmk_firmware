...
void matrix_init_custom(void) {
    i2c_init();
    uint8_t data = 0;
    i2c_status_t status = i2c_read_register(BUTTON_I2C_ADDRESS << 1, 0, &data, 1, BUTTON_I2C_TIMEOUT);
    is_connected = (status == I2C_STATUS_SUCCESS);
}

uint8_t button_get_pressed(void) {
    uint8_t data = 0;
    i2c_read_register(BUTTON_I2C_ADDRESS << 1, BUTTON_PRESSED, &data, 1, BUTTON_I2C_TIMEOUT);
    return data;
}

uint8_t button_get_clicked(void) {
    uint8_t data = 0;
    i2c_read_register(BUTTON_I2C_ADDRESS << 1, BUTTON_CLICKED, &data, 1, BUTTON_I2C_TIMEOUT);
    return data;
}

uint16_t button_get_debounce(void) {
    uint8_t data[2] = {0};
    i2c_read_register(BUTTON_I2C_ADDRESS << 1, BUTTON_DEBOUNCE, data, 2, BUTTON_I2C_TIMEOUT);
    return (data[0] << 8) | data[1];
}

bool button_get_pressed_interrupt(void) {
    uint8_t data = 0;
    i2c_status_t status = i2c_read_register(BUTTON_I2C_ADDRESS << 1, BUTTON_INTERRUPT, &data, 1, BUTTON_I2C_TIMEOUT);
